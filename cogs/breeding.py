...
class Breeding(commands.Cog):
    ...
    async def breed_command(self, ctx, count: int = 1):
        ...
        # Fetch settings and cooldowns in parallel
        ...
        # Convert cooldowns to set for O(1) lookups
        cooldown_ids = set(cooldowns.keys())
        
        # Single-pass filtering with gender pre-separation
        females = []
        males = []
        dittos = []
        ...
        # Pre-sort lists by IV once (highest first for most modes)
    ...
    def handle_all_breeding_opt(self, females, males, dittos, utils, selective, count):
        ...
        # FIXED: Pair remaining males with Ditto if still need more pairs
        if len(pairs) < count:
            for male in males:
                if len(pairs) >= count:
                    break
    ...
    def handle_tripzero_breeding_opt(self, females, males, dittos, utils, selective, count):
        ...
        # Pair females with lowest IV males
        ...
        # FIXED: Pair remaining males with Ditto (lowest IV)
        if len(pairs) < count:
            for male in males_low:
                ...
                # Find lowest IV available Ditto
    ...
    def handle_gmax_breeding_opt(self, females, males, dittos, utils, selective, count):
        ...
        # FIXED: Pair Gmax males with Ditto ONLY
        if len(pairs) < count:
    ...
    def handle_regionals_breeding_opt(self, females, males, dittos, utils, selective, count):
        ...
        # FIXED: Pair Regional males with Ditto ONLY
        if len(pairs) < count:
    ...
    async def handle_mychoice_breeding_opt(self, females, males, dittos, settings, utils, selective, count):
        ...
        # Pair matching females with matching males
    ...
    async def handle_specific_targets_breeding_opt(self, females, males, dittos, targets, utils, selective, count):
        ...
        for target in targets:
            for f in females:
                if self.matches_target(f, target, utils) and f not in filtered_females:
                    filtered_females.append(f)
            for m in males:
                if self.matches_target(m, target, utils) and m not in filtered_males:
                    filtered_males.append(m)

        if not filtered_females and not filtered_males:
            return []
        ...
        # FIXED: Step 2: Pair filtered males with Ditto
        if len(pairs) < count:
    ...
    def find_best_male_for_female_tripzero(self, female, males, dittos, utils, selective, used_male_ids):
        ...
        # Try compatible egg group (already sorted low to high)
        female_groups = female.get('egg_groups', [])
        for male in males:
            if male['pokemon_id'] in used_male_ids:
                continue
        ...
        # Try Ditto (already sorted low to high)
        for ditto in dittos:
            if ditto['pokemon_id'] not in used_male_ids:
                if self.can_pair_pokemon(female, ditto, utils, selective):
                    return ditto, 'ditto'
