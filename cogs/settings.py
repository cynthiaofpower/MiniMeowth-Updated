...
class Settings(commands.Cog):
    ...
    """
    DEBUGGED VERSION OF setfilter_command
    ======================================

    This version has extensive print statements to help debug issues.
    Replace your existing setfilter_command with this one.
    """

    @commands.hybrid_command(name='setfilter', aliases=['sf', 'breedfilter'])
    @app_commands.describe(
        gender="male or female",
        criteria="Filter criteria (e.g., --move fake out --spdiv 31)"
    )
    async def setfilter_command(self, ctx, gender: str = None, *, criteria: str = None):
        """Set breeding filter for male or female - DEBUGGED"""

        print("\n" + "="*80)
        print("DEBUG SETFILTER: Command called")
        print("="*80)

        user_id = ctx.author.id
        print(f"DEBUG SETFILTER: user_id = {user_id}")
        print(f"DEBUG SETFILTER: gender = {gender}")
        print(f"DEBUG SETFILTER: criteria = {criteria}")

        if not gender:
            print("DEBUG SETFILTER: No gender provided, showing current filters")
            await self.show_breed_filters(ctx)
            return

        gender = gender.lower()
        if gender not in ['male', 'female']:
            print(f"DEBUG SETFILTER: Invalid gender: {gender}")
            class ErrorView(discord.ui.LayoutView):
                container1 = discord.ui.Container(
                    discord.ui.TextDisplay(content="❌ Gender must be `male` or `female`"),
                )
            await ctx.send(view=ErrorView(), reference=ctx.message, mention_author=False)
            return

        # Handle special keywords
        if criteria and criteria.lower() == 'none':
            print(f"DEBUG SETFILTER: Disabling {gender} filter")
            await db.set_breed_filter_enabled(user_id, gender, False)

            class SuccessView(discord.ui.LayoutView):
                container1 = discord.ui.Container(
                    discord.ui.TextDisplay(
                        content=f"✅ **{gender.capitalize()} filter disabled**\n\n"
                                f"_Filter criteria saved. Use `m!setfilter {gender} enable` to re-enable._"
                    ),
                )
            await ctx.send(view=SuccessView(), reference=ctx.message, mention_author=False)
            return

        if criteria and criteria.lower() == 'remove':
            print(f"DEBUG SETFILTER: Removing {gender} filter")
            await db.remove_breed_filter(user_id, gender)

            class SuccessView(discord.ui.LayoutView):
                container1 = discord.ui.Container(
                    discord.ui.TextDisplay(content=f"✅ **{gender.capitalize()} filter removed completely**"),
                )
            await ctx.send(view=SuccessView(), reference=ctx.message, mention_author=False)
            return

        if criteria and criteria.lower() == 'enable':
            print(f"DEBUG SETFILTER: Enabling {gender} filter")
            await db.set_breed_filter_enabled(user_id, gender, True)

            class SuccessView(discord.ui.LayoutView):
                container1 = discord.ui.Container(
                    discord.ui.TextDisplay(content=f"✅ **{gender.capitalize()} filter enabled**"),
                )
            await ctx.send(view=SuccessView(), reference=ctx.message, mention_author=False)
            return

        if not criteria:
            print("DEBUG SETFILTER: No criteria provided")
            class ErrorView(discord.ui.LayoutView):
                container1 = discord.ui.Container(
                    discord.ui.TextDisplay(
                        content=f"❌ Please provide filter criteria\n\n"
                                f"**Examples:**\n"
                                f"{config.REPLY} `m!setfilter {gender} --move fake out --spdiv 31`\n"
                                f"{config.REPLY} `m!setfilter {gender} --iv >80 --fav`\n"
                                f"{config.REPLY} `m!setfilter {gender} none` (disable)\n"
                                f"{config.REPLY} `m!setfilter {gender} remove` (delete)"
                    ),
                )
            await ctx.send(view=ErrorView(), reference=ctx.message, mention_author=False)
            return

        # Parse the filter criteria using your existing Utils logic
        utils = self.bot.get_cog('Utils')
        if not utils:
            print("DEBUG SETFILTER: ERROR - Utils cog not loaded!")
            class ErrorView(discord.ui.LayoutView):
                container1 = discord.ui.Container(
                    discord.ui.TextDisplay(content="❌ Utils cog not loaded"),
                )
        ...
        print(f"DEBUG SETFILTER: Parsing criteria string: '{criteria}'")

        # Parse flags using your existing parse_add_flags method
        parsed = utils.parse_add_flags(criteria)

        print(f"DEBUG SETFILTER: Parsed result: {parsed}")
        print(f"DEBUG SETFILTER: Parsed keys: {list(parsed.keys())}")

        # Save to database
        print(f"DEBUG SETFILTER: Saving to database...")
        await db.set_breed_filter(user_id, gender, parsed, enabled=True)

        # Verify it was saved
        print(f"DEBUG SETFILTER: Verifying save...")
        verification = await db.get_breed_filters(user_id)
        print(f"DEBUG SETFILTER: Verification result: {verification}")

        saved_filter = verification.get(gender, {})
        print(f"DEBUG SETFILTER: Saved {gender} filter: {saved_filter}")
        print(f"DEBUG SETFILTER: Enabled: {saved_filter.get('enabled')}")
        print(f"DEBUG SETFILTER: Criteria: {saved_filter.get('criteria')}")

        # Build display of what was set
        filter_display = self._format_filter_criteria(parsed)

        print(f"DEBUG SETFILTER: Filter display: {filter_display}")
        print("="*80 + "\n")

        class SuccessView(discord.ui.LayoutView):
            container1 = discord.ui.Container(
                discord.ui.TextDisplay(
                    content=f"✅ **{gender.capitalize()} Filter Set**\n\n"
                            f"{filter_display}\n\n"
                            f"_Filter is now active. Use `m!settings target mychoice` to use this filter._\n\n"
                            f"**Debug Info:**\n"
    ...
    # ALSO ADD THIS DEBUGGED VERSION OF show_breed_filters

    async def show_breed_filters(self, ctx):
        """Display current breeding filters - DEBUGGED"""

        print("\n" + "="*80)
        print("DEBUG SHOW_FILTERS: Command called")
        print("="*80)

        user_id = ctx.author.id
        print(f"DEBUG SHOW_FILTERS: user_id = {user_id}")

        filters = await db.get_breed_filters(user_id)
        print(f"DEBUG SHOW_FILTERS: Raw filters from DB: {filters}")

        male_filter = filters.get('male', {})
        female_filter = filters.get('female', {})

        print(f"DEBUG SHOW_FILTERS: male_filter: {male_filter}")
        print(f"DEBUG SHOW_FILTERS: female_filter: {female_filter}")

        male_enabled = male_filter.get('enabled', False)
        female_enabled = female_filter.get('enabled', False)
        male_criteria = male_filter.get('criteria', {})
        female_criteria = female_filter.get('criteria', {})

        print(f"DEBUG SHOW_FILTERS: male_enabled: {male_enabled}")
        print(f"DEBUG SHOW_FILTERS: female_enabled: {female_enabled}")
        print(f"DEBUG SHOW_FILTERS: male_criteria: {male_criteria}")
        print(f"DEBUG SHOW_FILTERS: female_criteria: {female_criteria}")

        if male_criteria:
            male_display = self._format_filter_criteria(male_criteria)
            male_status = "🟢 Enabled" if male_enabled else "🔴 Disabled"
        else:
            male_display = "_Not set_"
            male_status = "⚪ No filter"

        if female_criteria:
            female_display = self._format_filter_criteria(female_criteria)
            female_status = "🟢 Enabled" if female_enabled else "🔴 Disabled"
        else:
            female_display = "_Not set_"
            female_status = "⚪ No filter"

        print(f"DEBUG SHOW_FILTERS: male_display: {male_display}")
        print(f"DEBUG SHOW_FILTERS: female_display: {female_display}")
        print("="*80 + "\n")

        # [Rest of the button code stays the same - just the display logic above]

        class FiltersView(discord.ui.LayoutView):
            container1 = discord.ui.Container(
                discord.ui.TextDisplay(content="**🔍 Breeding Filters (DEBUG MODE)**"),
                discord.ui.Separator(visible=True, spacing=discord.SeparatorSpacing.small),
                discord.ui.TextDisplay(
                    content=f"**{config.GENDER_MALE} Male Filter:** {male_status}\n{male_display}\n\n"
                            f"_Raw criteria keys: `{list(male_criteria.keys())}`_"
                ),
                discord.ui.Separator(visible=True, spacing=discord.SeparatorSpacing.small),
                discord.ui.TextDisplay(
                    content=f"**{config.GENDER_FEMALE} Female Filter:** {female_status}\n{female_display}\n\n"
                            f"_Raw criteria keys: `{list(female_criteria.keys())}`_"
                ),
            )

        await ctx.send(view=FiltersView(), reference=ctx.message, mention_author=False)


    # ALSO ADD DEBUGGED VERSION OF _format_filter_criteria

    def _format_filter_criteria(self, criteria: dict) -> str:
        """Format filter criteria for display"""

        if not criteria:
            return "_No criteria_"

        parts = []

        # Moves
        if 'moves' in criteria and criteria['moves']:
            parts.append(f"{config.REPLY} Moves: {', '.join(f'`{m}`' for m in criteria['moves'])}")

        # ===== NEW: NO MOVES =====
        if 'no_moves' in criteria and criteria['no_moves']:
            parts.append(f"{config.REPLY} Excluded Moves: {', '.join(f'`{m}`' for m in criteria['no_moves'])}")

        # ===== NEW: LEVEL =====
        if 'level' in criteria:
            level_filter = criteria['level']
            if 'exact' in level_filter:
                parts.append(f"{config.REPLY} Level: `{level_filter['exact']}`")
            else:
                parts.append(f"{config.REPLY} Level: `{level_filter['min']}-{level_filter['max']}`")

        # ===== NEW: FAVORITE =====
        if 'is_favorite' in criteria:
            fav_text = "Favorites only" if criteria['is_favorite'] else "Non-favorites only"
            parts.append(f"{config.REPLY} {fav_text}")

        # ===== NEW: NICKNAME =====
        if 'nickname' in criteria:
            parts.append(f"{config.REPLY} Nickname contains: `{criteria['nickname']}`")

        if 'no_nickname' in criteria:
            parts.append(f"{config.REPLY} Nickname excludes: `{criteria['no_nickname']}`")

        # Individual IVs
        iv_names = {'hpiv': 'HP', 'atkiv': 'Atk', 'defiv': 'Def', 
                    'spatkiv': 'SpAtk', 'spdefiv': 'SpDef', 'spdiv': 'Spd'}
        for iv_key, iv_label in iv_names.items():
            if iv_key in criteria:
                iv_range = criteria[iv_key]
                if iv_range['min'] == iv_range['max']:
                    parts.append(f"{config.REPLY} {iv_label}: `{iv_range['min']}`")
                else:
                    parts.append(f"{config.REPLY} {iv_label}: `{iv_range['min']}-{iv_range['max']}`")

        # Duplicate IVs
        if 'trip' in criteria:
            parts.append(f"{config.REPLY} Triple: `{criteria['trip']}`")
        if 'quad' in criteria:
            parts.append(f"{config.REPLY} Quadruple: `{criteria['quad']}`")
        if 'penta' in criteria:
            parts.append(f"{config.REPLY} Pentuple: `{criteria['penta']}`")
        if 'hex' in criteria:
            parts.append(f"{config.REPLY} Hextuple: `{criteria['hex']}`")

        return "\n".join(parts) if parts else "_Empty filter_"


    @commands.hybrid_command(name='viewfilters', aliases=['vf', 'breedfilters', 'bf'])
    async def viewfilters_command(self, ctx):
        """View current breeding filters"""
        await self.show_breed_filters(ctx)


    async def show_breed_filters(self, ctx):
        """Display current breeding filters with interactive buttons"""
        user_id = ctx.author.id
        filters = await db.get_breed_filters(user_id)
        ...
        class ToggleMaleButton(discord.ui.Button):
            def __init__(self, currently_enabled, has_filter):
                if not has_filter:
                    super().__init__(
                        style=discord.ButtonStyle.secondary,
                        label="No Male Filter",
                        disabled=True
                    )
                else:
                    super().__init__(
                        style=discord.ButtonStyle.success if currently_enabled else discord.ButtonStyle.secondary,
                        label="Disable Male" if currently_enabled else "Enable Male"
                    )
                self.currently_enabled = currently_enabled
            ...
            async def callback(self, interaction: discord.Interaction):
                if interaction.user.id != ctx.author.id:
                    class ErrorView(discord.ui.LayoutView):
                        container1 = discord.ui.Container(
                            discord.ui.TextDisplay(content="❌ This is not your filter view!"),
                        )
                    await interaction.response.send_message(view=ErrorView(), ephemeral=True)
                    return

                # Do work BEFORE responding
                new_state = not self.currently_enabled
                await db.set_breed_filter_enabled(interaction.user.id, 'male', new_state)
                ...
                class SuccessView(discord.ui.LayoutView):
                    container1 = discord.ui.Container(
                        discord.ui.TextDisplay(
                            content=f"✅ Male filter {state_text}"
                        ),
                    )

                # Send a new message (no defer needed)
                await interaction.response.send_message(view=SuccessView(), ephemeral=True)

        class ToggleFemaleButton(discord.ui.Button):
            def __init__(self, currently_enabled, has_filter):
                if not has_filter:
                    super().__init__(
                        style=discord.ButtonStyle.secondary,
                        label="No Female Filter",
                        disabled=True
                    )
                else:
                    super().__init__(
                        style=discord.ButtonStyle.success if currently_enabled else discord.ButtonStyle.secondary,
                        label="Disable Female" if currently_enabled else "Enable Female"
                    )
                self.currently_enabled = currently_enabled
                self.has_filter = has_filter
            ...
            async def callback(self, interaction: discord.Interaction):
                ...
                # Do work BEFORE responding
                new_state = not self.currently_enabled
                await db.set_breed_filter_enabled(interaction.user.id, 'female', new_state)
                ...
                class SuccessView(discord.ui.LayoutView):
                    container1 = discord.ui.Container(
                        discord.ui.TextDisplay(
                            content=f"✅ Female filter {state_text}"
                        ),
                    )
        ...
        class HelpButton(discord.ui.Button):
            ...
            async def callback(self, interaction: discord.Interaction):
                help_text = (
                    "**📚 Breeding Filter Guide**\n\n"
                    "**Setting Filters:**\n"
                    f"{config.REPLY} `m!setfilter male --move fake out --spdiv 31`\n"
                    f"{config.REPLY} `m!setfilter female --level >40 --fav`\n\n"
                    "**Available Filters:**\n"
                    f"{config.REPLY} `--level <value>` - Level (e.g., `50`, `>40`, `<=20`)\n"
                    f"{config.REPLY} `--fav` / `--unfav` - Favorite status\n"
                    f"{config.REPLY} `--nick <text>` - Nickname contains text\n"
                    f"{config.REPLY} `--nonick <text>` - Nickname must NOT contain text\n"
                    f"{config.REPLY} `--move <move>` - Has specific move\n"
                    f"{config.REPLY} `--nomove <move>` - Must NOT have specific move\n"
                    f"{config.REPLY} `--hpiv`, `--atkiv`, `--defiv`, `--spatkiv`, `--spdefiv`, `--spdiv` - Individual IVs\n"
                    f"{config.REPLY} `--trip`, `--quad`, `--penta`, `--hex` - Duplicate IVs\n\n"
                    "**Exclusion Filters:**\n"
                    f"{config.REPLY} `--nomove <move>` - Exclude Pokemon with this move\n"
                    f"{config.REPLY} `--nonick <text>` - Exclude Pokemon with this in nickname\n\n"
                    "**Managing Filters:**\n"
                    f"{config.REPLY} `m!setfilter male none` - Disable (keep criteria)\n"
                    f"{config.REPLY} `m!setfilter male enable` - Re-enable\n"
                    f"{config.REPLY} `m!setfilter male remove` - Delete completely\n\n"
                    "**How It Works:**\n"
                    f"{config.REPLY} If species set: filters narrow down those species\n"
                    f"{config.REPLY} If species NOT set: filters search ALL pokemon\n"
                    f"{config.REPLY} Set target to `mychoice` to use filters"
                )

                class HelpView(discord.ui.LayoutView):
                    container1 = discord.ui.Container(
                        discord.ui.TextDisplay(content=help_text),
                    )

                await interaction.response.send_message(view=HelpView(), ephemeral=True)

        # Build view
        class FiltersView(discord.ui.LayoutView):
            container1 = discord.ui.Container(
                discord.ui.TextDisplay(content="**🔍 Breeding Filters**"),
                discord.ui.Separator(visible=True, spacing=discord.SeparatorSpacing.small),
                discord.ui.TextDisplay(
                    content=f"**{config.GENDER_MALE} Male Filter:** {male_status}\n{male_display}"
                ),
                ...
                discord.ui.Separator(visible=True, spacing=discord.SeparatorSpacing.small),
                discord.ui.ActionRow(
                    ToggleMaleButton(male_enabled, bool(male_criteria)),
                    ToggleFemaleButton(female_enabled, bool(female_criteria)),
                    HelpButton()
                ),
            )
