...
async def _get_guild_doc(guild_id: int) -> dict:
    doc = await db.user_data.find_one({"user_id": _guild_key(guild_id)})
    return doc or {}

async def _save_guild_doc(guild_id: int, data: dict):
...
async def _get_enabled(guild_id: int) -> bool:
    doc = await _get_guild_doc(guild_id)
    return doc.get("incense_enabled", True)

async def _set_enabled(guild_id: int, value: bool):
    await _save_guild_doc(guild_id, {"incense_enabled": value})

async def _get_categories(guild_id: int) -> list[int]:
    doc = await _get_guild_doc(guild_id)
    return doc.get("incense_categories", [])

async def _set_categories(guild_id: int, cats: list[int]):
    await _save_guild_doc(guild_id, {"incense_categories": cats})

async def _get_paused_channels(guild_id: int) -> list[int]:
    doc = await _get_guild_doc(guild_id)
    return doc.get("incense_paused_channels", [])

async def _set_paused_channels(guild_id: int, channels: list[int]):
    await _save_guild_doc(guild_id, {"incense_paused_channels": channels})
...
async def _deny_poketwo_in_channel(channel: discord.TextChannel):
    """Deny Poketwo send_messages + view_channel in a single channel (used for single-channel pause)."""
...
async def _deny_poketwo_in_category(category: discord.CategoryChannel):
    ...
    # Set the perm on the category itself
...
async def _restore_poketwo_in_category(category: discord.CategoryChannel):
    ...
    # Remove the deny overwrite from the category
    overwrite = category.overwrites_for(poketwo)
...
FIELD_LIMIT = 1000   # stay safely under Discord's 1024 hard limit
FIELDS_PER_PAGE = 5  # max embed fields per page
...
def _build_list_pages(
    ...
    """
    Build a list of embeds (pages) for the inc list command.
    Each field value is capped at FIELD_LIMIT chars and we put at most
    FIELDS_PER_PAGE fields per embed so we never hit Discord limits.
    Returns (pages, total_channel_count).
    """
    icon = "⏸️" if showing_paused else "▶️"
    title = f"{icon} {'Paused' if showing_paused else 'Active'} Channels"

    # Collect all (category_name, channel_count, field_value_chunks) tuples
    # A single category may need to be split across multiple fields if it has
    # many channels whose mentions together exceed FIELD_LIMIT.
    ...
    for cat_id in cats:
        ...
        # Chunk lines so each field value stays under FIELD_LIMIT
        chunk: list[str] = []
        chunk_len = 0
        chunk_index = 0
        ...
        for line in lines:
            # +1 for the newline
            if chunk and chunk_len + len(line) + 1 > FIELD_LIMIT:
                cat_label = f"📁 {cat_obj.name} ({len(lines)})" if chunk_index == 0 else f"📁 {cat_obj.name} (cont.)"
                fields.append((cat_label, "\n".join(chunk)))
                chunk = []
                chunk_len = 0
                chunk_index += 1
            chunk.append(line)
    ...
    # Set footers with page numbers and total
    for idx, embed in enumerate(pages):
        embed.set_footer(
            text=f"Page {idx + 1}/{len(pages)} · Total: {total} channel{'s' if total != 1 else ''}"
        )
...
class IncenseListView(discord.ui.View):
    ...
    async def on_timeout(self):
        ...
        for item in self.children:
            item.disabled = True
        # Can't edit without a message reference here; discord.py handles cleanup
...
class Incense(commands.Cog):
    ...
    async def _channel_in_monitored_category(self, channel: discord.TextChannel) -> bool:
        cats = await _get_categories(channel.guild.id)
        return channel.category_id in cats

    async def _is_paused(self, channel: discord.TextChannel) -> bool:
        paused = await _get_paused_channels(channel.guild.id)
    ...
    async def _pause_channel(self, channel: discord.TextChannel):
        ...
        if channel.id not in paused:
            ...
            await _set_paused_channels(channel.guild.id, paused)
    ...
    async def on_message(self, message: discord.Message):
        ...
        if not isinstance(message.channel, discord.TextChannel):
            return
        if not await _get_enabled(message.guild.id):
            return
        ...
        if INCENSE_PATTERN.search(content):
            # Pause only the specific channel where incense was bought
    ...
    async def inc_toggle(self, interaction: discord.Interaction):
        current = await _get_enabled(interaction.guild_id)
        new_val = not current
        await _set_enabled(interaction.guild_id, new_val)
        embed = discord.Embed(
    ...
    async def inc_cat_add(self, interaction: discord.Interaction, category: discord.CategoryChannel):
        cats = await _get_categories(interaction.guild_id)
        ...
        await _set_categories(interaction.guild_id, cats)
    ...
    async def inc_pause(self, interaction: discord.Interaction, scope: str = None):
        ...
        if scope == "all":
            ...
            # Mark all channels in monitored categories as paused in DB
            paused = []
            for cat_id in cats:
                cat_obj = interaction.guild.get_channel(cat_id)
                if not isinstance(cat_obj, discord.CategoryChannel):
                ...
                for ch in cat_obj.text_channels:
                    paused.append(ch.id)
            await _set_paused_channels(interaction.guild_id, paused)
        ...
        # No scope = pause only the current channel
    ...
    async def inc_resume(self, interaction: discord.Interaction, scope: str = None):
        ...
        if scope == "all":
            ...
            await _set_paused_channels(interaction.guild_id, [])
        ...
        # No scope = resume the current channel
        channel = interaction.channel
        if not await self._channel_in_monitored_category(channel):
            return await interaction.followup.send(
                embed=discord.Embed(
    ...
    async def inc_list(self, interaction: discord.Interaction, status: str = "paused"):
        await interaction.response.defer()
        cats = await _get_categories(interaction.guild_id)
        paused_ids = set(await _get_paused_channels(interaction.guild_id))
    ...
    async def inc_help(self, interaction: discord.Interaction):
        p = config.PREFIX[0] if hasattr(config, "PREFIX") and config.PREFIX else "!"
        bot_mention = self._bot_mention()
        embed = discord.Embed(
            title="🔥 Incense Help",
            description=(
    ...
    async def inc_prefix_toggle(self, ctx: commands.Context):
        ...
        new_val = not current
        await _set_enabled(ctx.guild.id, new_val)
        embed = discord.Embed(
            title="Incense Watcher",
            description=f"The incense watcher is now {'**enabled** ✅' if new_val else '**disabled** 🔴'}.",
            color=config.EMBED_COLOR
    ...
    async def inc_prefix_cat_add(self, ctx: commands.Context, *, raw: str):
        ...
        cats = await _get_categories(ctx.guild.id)
        ...
        await _set_categories(ctx.guild.id, cats)
    ...
    async def inc_prefix_pause(self, ctx: commands.Context, target: str = None):
        ...
        if target and target.lower() == "all":
            ...
            # Mark all channels as paused in DB
            paused = []
            for cat_id in cats:
                cat_obj = ctx.guild.get_channel(cat_id)
                if not isinstance(cat_obj, discord.CategoryChannel):
                ...
                for ch in cat_obj.text_channels:
                    paused.append(ch.id)
            await _set_paused_channels(ctx.guild.id, paused)
        ...
        # No argument → pause the channel the command was run in
        channel = ctx.channel
    ...
    async def inc_prefix_resume(self, ctx: commands.Context, target: str = None):
        ...
        if target and target.lower() == "all":
            ...
            await _set_paused_channels(ctx.guild.id, [])
        ...
        # No argument → resume the channel the command was run in
        channel = ctx.channel
    ...
    async def inc_prefix_list(self, ctx: commands.Context, status: str = "paused"):
        ...
        cats = await _get_categories(ctx.guild.id)
        paused_ids = set(await _get_paused_channels(ctx.guild.id))
        showing_paused = (status == "paused")
